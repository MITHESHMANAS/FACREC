...
const getFacultyName = (faculty, facultyMap) => {
    if (!faculty) return "—";
    if (typeof faculty !== 'string') return String(faculty);
    const isObjectId = /^[0-9a-fA-F]{24}$/.test(faculty);
    if (!isObjectId) return faculty;
    return facultyMap[faculty] || "Not Assigned";
};
...
const AttendanceHistory = ({ history = [], facultyMap = {} }) => {
                ...
                <>
                    <div className="overflow-x-auto mt-4">
                        <table className="min-w-full text-sm">
                            ...
                            <tbody>
                                {rows.map((record) => (
                                    <tr key={record._id} className="border-t border-slate-100 hover:bg-slate-50 transition">
                                        <td className="px-6 py-3.5 text-slate-600">{record.session?.date || "—"}</td>
                                        <td className="px-6 py-3.5 font-medium text-slate-700">{record.session?.subject?.name || "N/A"}</td>
                                        <td className="px-6 py-3.5 text-slate-600">{getFacultyName(record.session?.faculty, facultyMap)}</td>
                                        <td className="px-6 py-3.5 text-center"><Badge status={record.status} /></td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
